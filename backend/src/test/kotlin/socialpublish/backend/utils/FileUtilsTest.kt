...
class FileUtilsTest {
    ...
    fun `sanitizeFilename should handle normal filenames`() {
        ...
        assertEquals("my_file.txt", FileUtils.sanitizeFilename("my_file.txt"))
        assertEquals("image-2024.png", FileUtils.sanitizeFilename("image-2024.png"))
    }

    @Test
    fun `sanitizeFilename should remove path separators`() {
        assertEquals("test.txt", FileUtils.sanitizeFilename("/path/to/test.txt"))
        assertEquals("test.txt", FileUtils.sanitizeFilename("..\\..\\test.txt"))
        assertEquals("file.doc", FileUtils.sanitizeFilename("../../sensitive/file.doc"))
    }

    @Test
    fun `sanitizeFilename should replace special characters with underscores`() {
        assertEquals("my_file_name.txt", FileUtils.sanitizeFilename("my file name.txt"))
        assertEquals("file_2024_.pdf", FileUtils.sanitizeFilename("file@2024!.pdf"))
        assertEquals("document_.doc", FileUtils.sanitizeFilename("document*.doc"))
    }

    @Test
    fun `sanitizeFilename should normalize Unicode characters`() {
        // Test with accented characters - they should be normalized
        val result = FileUtils.sanitizeFilename("café.txt")
    ...
    fun `sanitizeFilename should remove leading invalid characters`() {
        ...
        assertEquals("file.txt", FileUtils.sanitizeFilename("---file.txt"))
        assertEquals("hidden.txt", FileUtils.sanitizeFilename(".hidden.txt"))
    }

    @Test
    fun `sanitizeFilename should generate UUID for completely invalid names`() {
        val result1 = FileUtils.sanitizeFilename("...")
        val result2 = FileUtils.sanitizeFilename("///")
        val result3 = FileUtils.sanitizeFilename("@@@")
    ...
    fun `sanitizeFilename should prevent header injection`() {
        ...
        assertEquals("file__name.txt", FileUtils.sanitizeFilename("file\r\nname.txt"))
        assertEquals("test_.pdf", FileUtils.sanitizeFilename("test\".pdf"))
        assertEquals("doc_.txt", FileUtils.sanitizeFilename("doc'.txt"))
    }
    ...
    fun `isPathWithinBase should allow files in base directory`(@TempDir tempDir: File) {
        ...
        assertTrue(FileUtils.isPathWithinBase(file, baseDir))
    }
    ...
    fun `isPathWithinBase should reject files outside base directory`(@TempDir tempDir: File) {
        ...
        assertFalse(FileUtils.isPathWithinBase(outsideFile, baseDir))
    }
    ...
    fun `isPathWithinBase should reject path traversal attempts`(@TempDir tempDir: File) {
        ...
        assertFalse(FileUtils.isPathWithinBase(traversalFile, baseDir))
    }
    ...
    fun `isPathWithinBase should prevent base path prefix confusion`(@TempDir tempDir: File) {
        ...
        assertFalse(FileUtils.isPathWithinBase(maliciousFile, appDir))
    }
    ...
    fun `isPathWithinBase should handle base directory itself`(@TempDir tempDir: File) {
        ...
        assertTrue(FileUtils.isPathWithinBase(baseDir, baseDir))
    }
}
