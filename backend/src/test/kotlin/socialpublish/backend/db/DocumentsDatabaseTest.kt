...
class DocumentsDatabaseTest {
    ...
    @Test
    fun `createOrUpdate should create new document`(@TempDir tempDir: Path) =
        runTest {
    ...
    @Test
    fun `createOrUpdate should update existing document by searchKey`(
    ...
    @Test
    fun `createOrUpdate should generate searchKey if not provided`(
        @TempDir tempDir: Path
    ) = runTest {
        val dbPath = tempDir.resolve("test.db").toString()
    ...
    @Test
    fun `searchByKey should find document by searchKey`(
        @TempDir tempDir: Path
    ) = runTest {
        val dbPath = tempDir.resolve("test.db").toString()
        ...
        resourceScope {
            ...
            // Search for it
            val found =
                documentsDb.searchByKey("find-me", userA).getOrElse { throw it }
    ...
    @Test
    fun `searchByKey should return null for non-existent key`(
        @TempDir tempDir: Path
    ) = runTest {
        val dbPath = tempDir.resolve("test.db").toString()
        ...
        resourceScope {
            ...
            val notFound =
                documentsDb.searchByKey("does-not-exist", userA).getOrElse {
                    throw it
                }
    ...
    @Test
    fun `searchByKey should only return document for the requested user`(
        @TempDir tempDir: Path
    ) = runTest {
        val dbPath = tempDir.resolve("test.db").toString()
        ...
        resourceScope {
            ...
            val foundForOwner =
                documentsDb.searchByKey("shared-key", userA).getOrElse {
                    throw it
                }
            val foundForOtherUser =
                documentsDb.searchByKey("shared-key", userB).getOrElse {
                    throw it
                }
    ...
    @Test
    fun `searchByUuid should find document by UUID`(@TempDir tempDir: Path) =
        runTest {
            val dbPath = tempDir.resolve("test.db").toString()
            ...
            resourceScope {
                ...
                // Search by UUID
                val found =
                    documentsDb.searchByUuid(created.uuid).getOrElse {
                        throw it
                    }
    ...
    @Test
    fun `searchByUuid should return null for non-existent UUID`(
        @TempDir tempDir: Path
    ) = runTest {
        val dbPath = tempDir.resolve("test.db").toString()
        ...
        resourceScope {
            ...
            val notFound =
                documentsDb
                    .searchByUuid("00000000-0000-0000-0000-000000000000")
                    .getOrElse { throw it }
    ...
    @Test
    fun `getAll should retrieve all documents of a kind`(
        @TempDir tempDir: Path
    ) = runTest {
        val dbPath = tempDir.resolve("test.db").toString()
    ...
    @Test
    fun `getAll should return documents in CREATED_AT_DESC order`(
        @TempDir tempDir: Path
    ) = runTest {
        val dbPath = tempDir.resolve("test.db").toString()
        ...
        resourceScope {
            ...
            val first =
                documentsDb
                    ...
                    .getOrElse { throw it }
            // Small delay to ensure different timestamps (DB stores millis
            // precision)
    ...
    @Test
    fun `getAll should return empty list for non-existent kind`(
        @TempDir tempDir: Path
    ) = runTest {
        val dbPath = tempDir.resolve("test.db").toString()
    ...
    @Test
    fun `tags should be persisted and retrieved correctly`(
        @TempDir tempDir: Path
    ) = runTest {
        val dbPath = tempDir.resolve("test.db").toString()
        ...
        resourceScope {
            ...
            val retrieved =
                documentsDb.searchByUuid(created.uuid).getOrElse { throw it }
    ...
    @Test
    fun `updating document should replace tags`(@TempDir tempDir: Path) =
        runTest {
            val dbPath = tempDir.resolve("test.db").toString()
