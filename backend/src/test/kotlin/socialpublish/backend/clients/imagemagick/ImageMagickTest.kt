...
class ImageMagickTest {
    ...
    fun setup() {
        imageMagick = runBlocking {
            ImageMagick().getOrElse {
                error(
                    "ImageMagick not available: ${it.message}. Is ImageMagick installed?"
                )
            }
        }
    ...
    @Test
    fun `identifyImageSize should work for second test image`(
        @TempDir tempDir: Path
    ) = runBlocking {
    ...
    @Test
    fun `identifyImageSize should fail for non-existent file`(
        @TempDir tempDir: Path
    ) = runBlocking {
        ...
        assertTrue(
            result.isLeft(),
            "identifyImageSize should fail for non-existent file",
        )
    ...
    @Test
    fun `identifyImageSize should fail for non-image file`(
        @TempDir tempDir: Path
    ) = runBlocking {
        ...
        assertTrue(
            result.isLeft(),
            "identifyImageSize should fail for non-image file",
        )
    }

    @Test
    fun `optimizeImage should create optimized JPEG file`(
        @TempDir tempDir: Path
    ) = runBlocking {
        ...
        assertTrue(
            sizeResult.isRight(),
            "Should be able to identify optimized image",
        )
        ...
        assertTrue(
            size.width < 4966,
            "Image should be resized from original width",
        )
        assertTrue(
            size.height < 3313,
            "Image should be resized from original height",
        )
    }

    @Test
    fun `optimizeImage should respect custom size constraints`(
        @TempDir tempDir: Path
    ) = runBlocking {
        val customMagick =
            ImageMagick(
        ...
        assertTrue(
            result.isRight(),
            "optimizeImage with custom options should succeed",
        )
        assertTrue(dest.exists(), "Destination file should be created")
        ...
        assertTrue(
            size.width <= 800,
            "Width should be <= custom max width (800)",
        )
        assertTrue(
            size.height <= 600,
            "Height should be <= custom max height (600)",
        )
        assertTrue(dest.length() <= 500_000, "File size should be <= 500KB")
    }

    @Test
    fun `optimizeImage should enforce maximum file size`(
        @TempDir tempDir: Path
    ) = runBlocking {
        ...
        assertTrue(
            result.isRight(),
            "optimizeImage should succeed with reasonable constraints",
        )
        assertTrue(dest.exists(), "Destination file should be created")
        assertTrue(
            dest.length() <= 200_000,
    ...
    @Test
    fun `optimizeImage should fail when size constraints cannot be met`(
        @TempDir tempDir: Path
    ) = runBlocking {
    ...
    @Test
    fun `optimizeImage should fail when source does not exist`(
        @TempDir tempDir: Path
    ) = runBlocking {
        ...
        assertTrue(
            result.isLeft(),
            "optimizeImage should fail for non-existent source",
        )
        val error = result.leftOrNull()!!
        // The error message could be from validateFiles or from the ImageMagick
        // command failing
        assertTrue(
            error.message?.contains("does not exist") == true ||
    ...
    @Test
    fun `optimizeImage should fail when destination already exists`(
        @TempDir tempDir: Path
    ) = runBlocking {
        ...
        assertTrue(
            result.isLeft(),
            "optimizeImage should fail when destination exists",
        )
        val error = result.leftOrNull()!!
        assertTrue(
    ...
    @Test
    fun `optimizeImage should preserve aspect ratio`(@TempDir tempDir: Path) =
        runBlocking {
            val dest = tempDir.resolve("optimized-aspect.jpg").toFile()

            // Get original dimensions
            val originalSize =
                imageMagick.identifyImageSize(testFlower1).getOrNull()!!
            val originalRatio =
                originalSize.width.toDouble() / originalSize.height.toDouble()
            ...
            val optimizedSize =
                imageMagick.identifyImageSize(dest).getOrNull()!!
            val optimizedRatio =
                optimizedSize.width.toDouble() / optimizedSize.height.toDouble()

            // Aspect ratio should be preserved (within 1% tolerance for
            // rounding)
            val ratioDiff =
                kotlin.math.abs(originalRatio - optimizedRatio) / originalRatio
            assertTrue(
                ratioDiff < 0.01,
    ...
    fun `optimizeImage should not resize image smaller than max dimensions`(
        ...
        val largeDimMagick =
            ImageMagick(
                    options =
                        MagickOptimizeOptions(maxWidth = 5000, maxHeight = 5000)
                )
                .getOrElse { error("ImageMagick not available: ${it.message}") }

        val dest = tempDir.resolve("optimized-no-resize.jpg").toFile()
        val originalSize =
            largeDimMagick.identifyImageSize(testFlower1).getOrNull()!!
    ...
    @Test
    fun `ImageMagick companion invoke should find magick executable`() =
        runBlocking {
            val result = ImageMagick()

            assertTrue(
                result.isRight(),
                "ImageMagick() should successfully locate magick executable",
            )
            assertNotNull(
                result.getOrNull(),
                "Should return ImageMagick instance",
            )
            Unit
        }
    ...
    fun `ImageMagick companion invoke should fail gracefully when magick not found`() =
        runBlocking {
            // This test would require mocking the `which` command, which is
            // complex
    ...
    @Test
    fun `optimizeImage should respect EXIF orientation and auto-rotate`(
        @TempDir tempDir: Path
    ) = runBlocking {
        ...
        // Get original dimensions (without auto-orient, this would be
        // 4000x3000)
        ...
        // After auto-orient, it should be physically rotated to width=3000,
        // height=4000
