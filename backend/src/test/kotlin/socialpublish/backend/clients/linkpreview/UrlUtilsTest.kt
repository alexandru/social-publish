...
class UrlUtilsTest {
    ...
    fun `resolves absolute HTTP URLs unchanged`() {
        val result =
            resolveImageUrl("http://example.com/image.jpg", "https://base.com")
    ...
    fun `resolves protocol-relative URLs with HTTPS base`() {
        val result =
            resolveImageUrl("//cdn.example.com/image.jpg", "https://base.com")
    ...
    fun `resolves protocol-relative URLs with HTTP base`() {
        val result =
            resolveImageUrl("//cdn.example.com/image.jpg", "http://base.com")
    ...
    fun `resolves root-relative URLs with standard HTTP port`() {
        val result =
            resolveImageUrl("/images/photo.jpg", "http://example.com:80/blog")
    ...
    fun `resolves root-relative URLs with standard HTTPS port`() {
        val result =
            resolveImageUrl("/images/photo.jpg", "https://example.com:443/blog")
    ...
    fun `resolves path-relative URLs from file`() {
        val result =
            resolveImageUrl("image.jpg", "https://example.com/blog/post.html")
        assertEquals("https://example.com/blog/image.jpg", result)
    }
    ...
    fun `resolves path-relative URLs with parent directory navigation`() {
        val result =
            resolveImageUrl(
                "../assets/image.jpg",
                "https://example.com/blog/posts/post.html",
            )
        assertEquals("https://example.com/blog/assets/image.jpg", result)
    }
    ...
    fun `handles fragments in image URL`() {
        val result =
            resolveImageUrl("/image.svg#icon", "https://example.com/blog")
    ...
    fun `resolves URLs with special characters`() {
        val result =
            resolveImageUrl("/images/café photo.jpg", "https://example.com")
    ...
    fun `handles base URL with path and query parameters`() {
        val result =
            resolveImageUrl(
                "image.jpg",
                "https://example.com/blog/post?id=123&ref=home",
            )
        assertEquals("https://example.com/blog/image.jpg", result)
    }
