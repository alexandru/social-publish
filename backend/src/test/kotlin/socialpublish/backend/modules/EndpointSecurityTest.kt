...
class EndpointSecurityTest {
    ...
    fun `different users should have different tokens`() {
        ...
        val token1 = authModule.generateToken("user1", UUIDv7.generate())
        val token2 = authModule.generateToken("user2", UUIDv7.generate())
    ...
    fun `tampered token should be rejected`() {
        val authModule = AuthModule(config.jwtSecret)
        val validToken = authModule.generateToken("testuser", UUIDv7.generate())
