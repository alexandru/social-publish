 ...
 *
 * `FromFile` is a "cold source", meaning the client can read from it multiple
 * times. `FromSource` is a "hot source", meaning the client should read from it
 * only once.
 */
sealed interface UploadSource {
     ...
     *
     * If the source is `FromFile`, it returns the existing file. If the source
     * is FromSource, it creates a temporary file and saves the source to it.
     *
     * The returned Resource<File> will manage the lifecycle of the temporary
     * file, deleting it when the resource is closed.
     */
    fun asFileResource(): Resource<File> = let { uploadSource ->
        resource {
            when (uploadSource) {
                ...
                is FromSource ->
                    createTempFileResource("source-").bind().also {
                        uploadSource.source.saveToFile(it)
                    }
            }
        }
    }
    ...
    suspend fun readBytes(): ByteArray = resourceScope {
        asKotlinSource().bind().use { source -> source.readByteArray() }
    }
}
 ...
 * - Removes all other characters including whitespace, quotes, and control
 *   characters
...
fun sanitizeFilename(filename: String): String {
    ...
    return withoutLeadingInvalid.ifBlank { UUID.randomUUID().toString() }
}

/**
 * Validate that a file path is within an allowed base directory.
 *
 * This prevents path traversal attacks by ensuring the canonical path of the
 * file is within the canonical path of the base directory.
 *
...
fun isPathWithinBase(file: File, baseDir: File): Boolean {
    ...
    return canonicalFile.path == canonicalBaseDir.path ||
        canonicalFile.path.startsWith(allowedPath)
}
...
suspend fun Source.forEachChunk(
    chunkSize: Int = 8192,
    block: suspend (ByteArray, Int) -> Unit,
) {
...
/**
 * Perform operations on a file, creating a temporary backup copy, to be
 * restored in case of exceptions.
 */
            ...
            if (source.exists()) {
                val _ =
                    install(
                        {
                            val tempFile =
                                createTempFileName("backup-", source.name)
                            runInterruptible(Dispatchers.LoomIO) {
                        ...
                        { tempFile, exitCase ->
                            runInterruptible(Dispatchers.LoomIO) {
                                if (exitCase != ExitCase.Completed) {
                                    // Restore original file from temp copy in
                                    // case of error
