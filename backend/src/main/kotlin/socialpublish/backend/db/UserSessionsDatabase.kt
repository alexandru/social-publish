...
class UserSessionsDatabase(private val db: Database, private val usersDb: UsersDatabase) {
    ...
    suspend fun deleteSession(tokenHash: String): Either<DBException, Boolean> = db.transaction {
        val deleted =
    ...
    suspend fun deleteExpiredSessions(): Either<DBException, Int> = db.transaction {
        val now = db.clock.instant()
