...
class UsersDatabase(private val db: Database) {
     ...
     */
    suspend fun findByUsername(username: String): Either<DBException, User?> = db.transaction {
     ...
     * @return Either a DBException or the User (null if not found)
     */
    suspend fun findByUuid(uuid: UUIDv7): Either<DBException, User?> = db.transaction {
        ...
        when (
            ...
            is Either.Left ->
                when (val error = result.value) {
                    ...
                    else -> DBException(error.message ?: "Failed to update username", error).left()
                }
