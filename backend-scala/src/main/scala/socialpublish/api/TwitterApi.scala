...
private case class AuthorizedToken(key: String, secret: String) derives Codec.AsObject
private case class TwitterMediaUploadResponse(media_id_string: String)
private case class CreateNewPostRequest(text: String, media: Option[MediaIds])
private case class MediaIds(media_ids: List[String])
private case class TweetData(id: String)
private case class TweetResponse(data: TweetData)
...
private class TwitterApiImpl(
  ...
  override def getAuthorizationUrl(jwtAccessToken: String): Result[String] =
    val callbackUrl = s"${config.baseUrl}/api/twitter/callback?access_token=${URLEncoder.encode(jwtAccessToken, "UTF-8")}"
    val reqUrl = s"$requestTokenURL?oauth_callback=${URLEncoder.encode(callbackUrl, "UTF-8")}&x_auth_access_type=write"
  ...
  override def createPost(postReq: NewPostRequest): Result[NewPostResponse] =
    for
      ...
      // Prepare text
      content = if postReq.cleanupHtml.getOrElse(false) then
      ...
      // Create tweet
      tweetData = if mediaIds.nonEmpty then
        CreateNewPostRequest(text, Some(MediaIds(mediaIds)))
      else
  ...
  private def generateSignature(
    ...
    val signingKey = token match
      case Some(t) => s"${percentEncode(config.twitterOauth1ConsumerSecret)}&${percentEncode(t.secret)}"
