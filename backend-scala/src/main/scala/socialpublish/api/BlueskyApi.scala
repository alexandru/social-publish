...
private class BlueskyApiImpl(
  ...
  private case class BlobRef(`$type`: String, ref: Json, mimeType: String)
  private case class ImageEmbed(alt: String, image: BlobRef, aspectRatio: Option[AspectRatio])
  ...
  private case class Facet(index: ByteSlice, features: List[Feature])
  private case class ByteSlice(byteStart: Int, byteEnd: Int)
  private case class Feature(`$type`: String, uri: String)
  ...
  private def createPostRecord(
    ...
    val embed = if images.nonEmpty then
      Some(Json.obj(
