...
val Account =
  FC<Props> {
    ...
    val authorizeTwitter = { window.location.href = "/api/twitter/authorize" }
      ...
      scope.launch {
        ...
        if (response.status == 401.toShort() || response.status == 403.toShort()) {
          navigate(
            navigateOptionsWithSearch(
              to = "/login",
              searchParams = mapOf("error" to "${response.status}", "redirect" to "/account"),
            )
          )
          return@launch
        }
        if (response.status == 200.toShort()) {
          ...
          val hasAuthorization =
            bodyJson?.get("hasAuthorization")?.jsonPrimitive?.booleanOrNull ?: false
          if (hasAuthorization) {
            val createdAt = bodyJson["createdAt"]?.jsonPrimitive?.contentOrNull
            val atDateTime =
              if (createdAt != null) " at ${Date(createdAt).toLocaleString()}" else ""
