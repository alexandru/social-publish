...
class TwitterApiModule(
  ...
  private fun createOAuthService(callback: String? = null): OAuth10aService {
    val builder = ServiceBuilder(config.oauth1ConsumerKey).apiSecret(config.oauth1ConsumerSecret)
    if (callback != null) {
  ...
  suspend fun saveOauthToken(token: String, verifier: String): ApiResult<Unit> {
    return try {
      ...
      val accessToken = withContext(Dispatchers.IO) { service.getAccessToken(reqToken, verifier) }
  ...
  suspend fun statusRoute(call: ApplicationCall) {
    ...
    call.respond(
      mapOf("hasAuthorization" to (row != null), "createdAt" to row?.createdAt?.toEpochMilli())
    )
  }
