...
class FilesDatabase(private val jdbi: Jdbi) {
    ...
    fun createFile(payload: UploadPayload): Upload {
        return jdbi.inTransaction<Upload, Exception> { handle ->
            ...
            val existing = handle.createQuery("SELECT * FROM uploads WHERE uuid = ?")
                .bind(0, uuid)
                .mapToMap()
                .findOne()
                .orElse(null)
            ...
            handle.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """.trimIndent(),
                upload.uuid, upload.hash, upload.originalname, upload.mimetype, 
                upload.size, upload.altText, upload.imageWidth, upload.imageHeight, 
    ...
    fun getFileByUuid(uuid: String): Upload? {
        return jdbi.withHandle<Upload?, Exception> { handle ->
            val row = handle.createQuery("SELECT * FROM uploads WHERE uuid = ?")
                .bind(0, uuid)
                .mapToMap()
                .findOne()
